...
const Home = () => {
  ...
  const [showFilters, setShowFilters] = useState(width < 768 ? false : true);
  ...
  return (
    <StyledWrapper className="min-h-screen h-100 bg-slate-900">
      <Header />
      <section className="max-w-screen-2xl ml-auto mr-auto lg:mt-20 mt-6 gap-6 flex flex-col md:flex-row px-3">
        {width < 768 && (
          <Button variant="second" onClick={() => setShowFilters(!showFilters)}>
            Show filter options
          </Button>
        )}
        {showFilters && (
          <aside>
            <div className="flex flex-col bg-slate-800 p-4 rounded-lg gap-2">
              <p className="text-white mb-2 text-xs">
                ...
                remaining weights will be distributed among the other
                properties.
              </p>
              <ul
        ...
        <main className="w-full">
          ...
          <ul className="grid grid-cols-1  lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
            {flakes.data &&
